...
- Keep authored `.mjs` files in `src/runtime/` source-compatible with both TypeScript consumers and the copied production entry; Module Builder emits them as `.js`, and `src/build.ts` restores the specifier names in production output.
