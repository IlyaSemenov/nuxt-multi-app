...
export function createChild(
  ...
  async function start(server: Server) {
    ...
    state = { type: "ready" }
    console.log(`[nuxt-multi-app] mounted ${options.id}; root=${options.rootDir}`)
  }
  ...
  return {
    ...
    start(server: Server) {
      ...
      return (starting ??= start(server).catch((error) => {
        ...
        console.error(`[nuxt-multi-app:${options.id}]`, error)
        throw error
      }))
    },
    ...
    async upgrade(request: Parameters<RequestListener>[0], socket: Duplex, head: Buffer) {
      ...
      if (request.headers["sec-websocket-protocol"] === "vite-hmr") {
        if (debug) console.log(`[nuxt-multi-app:${options.id}] HMR upgrade ${request.url}`)
    ...
    close() {
      ...
      return (closing = (async () => {
        ...
        await nuxt?.close()
        console.log(`[nuxt-multi-app] closed ${options.id}`)
      })())
    },
  }
}
